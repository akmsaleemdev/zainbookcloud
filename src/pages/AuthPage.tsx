    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {mode === "signup" && (
            <div className="space-y-2">
              ...
              <Input id="fullName" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Your full name" className="h-12 text-[15px] bg-secondary/50 border-border/30 rounded-xl" required />
            </div>
          )}
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" className="pl-12 h-12 text-[15px] bg-secondary/50 border-border/30 rounded-xl" required />
            </div>
          </div>
          {mode !== "forgot" && (
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="pl-12 pr-12 h-12 text-[15px] bg-secondary/50 border-border/30 rounded-xl" required minLength={6} />
          ...
          <Button type="submit" className="w-full h-12 text-[15px] btn-premium rounded-xl font-semibold" disabled={loading}>
        ...
        <div className="text-[15px] text-center text-muted-foreground mt-8">
          {mode === "forgot" ? (
            <button onClick={() => setMode("login")} className="text-primary hover:underline font-medium flex items-center gap-1.5 mx-auto">
